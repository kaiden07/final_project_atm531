...
ZONE_DIRS = {z: HVAC_ROOT / f"Zone{z}" for z in "IJK"}
for _d in ZONE_DIRS.values():
...
def process_year(year: int, dom: str, max_days: int | None):
    ...
    for zone in "IJK":
        p = ZONE_DIRS[zone] / f"compare_hvac_baf_Zone{zone}_{year}.csv"
        fh = open(p, "w", newline="")
    ...
    try:
        ...
        _finish(year, all_rows, zone_files)
...
def plot_year(year: int, rows: list[dict]):
    ...
    for zone in "IJK":
        ...
        png_path = ZONE_DIRS[zone] / f"compare_hvac_baf_Zone{zone}_{year}.png"
...
def main():
    ...
    for yr in args.years:
        log.info("=" * 60)
        log.info("Year %d  (domain=%s)", yr, args.domain)
        log.info("=" * 60)
        process_year(yr, args.domain, args.max_days)

    log.info("Done. Output in %s", HVAC_ROOT)
